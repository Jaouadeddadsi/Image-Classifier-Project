...
def load_checkpoint(filename='checkpoint.pth'):
    ...
    classifier = nn.Sequential(OrderedDict([ ('fc1', nn.Linear(25088, checkpoint['hidden_units'],  bias=True)),
                                                ('Relu1', nn.ReLU()),
                                                ('Dropout1', nn.Dropout(p = 0.5)),
                                                ('fc2', nn.Linear(checkpoint['hidden_units'], 102,  bias=True)),
...
def process_image(image_path):
    ...
    return np_im
def predict(image_path, model, topk=5, category_names = 'cat_to_name.json', device = 'cuda'):
...
if __name__ == '__main__' :
    ...
    probs, flowers = predict(args.image_path, model, topk= topk, category_names = category_names, device = device)
    df = pd.DataFrame({'probability':probs, 'flower': flowers})
    df.to_csv('results.csv')
    #view_classify(args.image_path, probs, flowers)
